...
//averaging rating 
  ...
  $qur1 = "select count(*) as dd, avg(rating) as xx from attractionRatings where attraction_id='$attraction_id' group by attraction_id";
...
<table cellpadding=0 cellspacing=0 border=0 style="font-size: 13px;">
   <tr align=left>
      <td>
        <form name=rate method=post action="starAttractionRating.php?attraction_id=<?php echo($attraction_id); ?>">
             <b>This attraction is currently rated as: </b>
             <?php for($i=1;$i<=5;$i++)
                     {
                   	if($rating>=1)
                	{
...
$checkexist = mysql_result(mysql_query("Select count(*) from attractionRatings where user_id='".$_SESSION['user_id']."' && attraction_id='$attraction_id'"), 0);
